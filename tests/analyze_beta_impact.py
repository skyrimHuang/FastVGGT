"""
分析β参数对合并率的影响：基于单调上升调制，快速定位最优β范围。
...
def analyze_beta_range() -> dict:
    """
    分析不同β值的合理范围：
    - β过小：序列长度对合并率影响不足，长序列加速不明显
    - β过大：高S区域过快触及r_max，精度风险增大
    """
    # 4层锚点值（来自Optimal配置）
    ...
    # 测试不同β值
    beta_candidates = [0.0001, 0.0005, 0.001, 0.002, 0.003, 0.005]
    ...
    for beta in beta_candidates:
        ...
        r_changes = []
        for s in s_typical:
            growth = 1.0 + beta * max(0, s - s_base)
            # 以layer 16（中间层）的C_base=0.8为例
            r_mid = min(0.9, max(0.0, 0.8 * growth))
...
def main():
    ...
    我们分析了β∈[0.0001, 0.005]范围内的影响：
    - β过小(<0.0005)会导致长序列上调幅度不足，加速优势不明显
    - β过大(≥0.005)会使高S区域更快触及r_max，存在精度风险
    - 推荐范围为β∈[0.001, 0.003]，可在速度收益与精度稳定性间取得较好平衡"
    """)
